...
## Dimension 범위 검증 (버그 수정)

### 발견된 버그
M/N/K를 3비트로 잘라 저장하던 탓에, 범위 밖 값이 aliasing되어 검증을
우회했다. 예: M=9 (1001) -> 하위 3비트 001 = 1로 저장 -> valid처럼 보여
잘못된 연산 수행. (M=5는 101=5로 남아 invalid 검출됐으나 8,9,10,11 등은 통과)

### 수정 방식: 저장은 좁게, 검사는 32비트 전체
- dim 레지스터는 3비트 유지 (곱셈 인덱스 폭 불변 -> 면적/PPA 영향 없음)
- mmio_reg가 write 시점에 **잘리지 않은 32비트 wdata**로 [1,4] 범위 검사
- 범위 밖이면 dim_oor sticky 플래그 -> FSM의 dims_ok 판정에 반영

```verilog
wire wdata_in_range = (mmio_wdata >= 32'd1) && (mmio_wdata <= 32'd4);
// M/N/K write 시 r_*_oor <= ~wdata_in_range
// dim_oor = r_m_oor | r_n_oor | r_k_oor  -> FSM에서 ~dim_oor로 AND
```

### 검증 (tb_dim_edge)
- 9,10,11,12,5,8,15 -> 전부 invalid_size + 연산 안 함
- 17,20,33,65536 (4비트로는 alias됐을 값) -> 전부 invalid 검출
- 0 (하한 위반) -> invalid 검출
- 유효값 1~4 -> 정상 동작
