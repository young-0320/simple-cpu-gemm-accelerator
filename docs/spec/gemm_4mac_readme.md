...
CPU freeze/resume -> status 확인 -> 메모리의 C 결과 확인까지 전 과정.
...
### 왜 stall이 없나
GEMM busy 동안 current integrated RTL은 CPU를 freeze하므로 instruction fetch,
normal memory access, status polling cycle이 모두 발생하지 않는다. 그래서 LSU는
BRAM을 독점하고 한 cycle도 밀리지 않는다. 멈추는 것은 놀고 있던 CPU이지 일하는 LSU가 아니다.
...
```asm
    ...
    STORE 0xFF6
CHECK_STATUS:
    LOAD  0xFF7      ; final status after CPU resume
    CMPI  0x2        ; done?
    JZ    FINISH
    JMP   CHECK_STATUS
FINISH:
...
Current integrated RTL에서는 GEMM busy 동안 CPU가 freeze되므로 `CHECK_STATUS`
loop가 busy cycle마다 반복 실행되지 않는다. CPU는 GEMM 완료 후 resume되어
final status를 확인한다.
...
4. busy=0 -> cpu_run=1 (CPU 재개) -> STATUS 확인 -> out_port=0x8
