...
CPU가 GEMM을 시작하면 memory data path의 주인은 GEMM accelerator이다. Current integrated RTL은 busy 동안 CPU를 freeze하므로 CPU는 normal memory access나 MMIO status polling cycle을 발행하지 않는다. CPU는 GEMM 완료 후 resume되어 final status를 읽는다.
