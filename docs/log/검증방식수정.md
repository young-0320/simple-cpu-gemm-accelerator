...
이번 작업의 목적은 GEMM accelerator의 최적화 방향을 단순히 RTL 구조 변경으로만 설명하는 것이 아니라, testbench에서 관찰 가능한 cycle 수를 근거로 비교할 수 있게 만드는 것이다.
...
따라서 RTL 통합은 다음 작업으로 넘기고, 오늘 작업은 검증 방식 정리와 비교 기준 확보까지 완료된 것으로 보는 것이 적절하다.
