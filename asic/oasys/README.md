...
1. Oasys 실행 후 새 config를 만든다.
2. 합성할 Verilog RTL 파일들을 추가한다.
3. top module 이름을 지정한다.
4. timing library file을 추가한다.
5. physical library와 technology file을 추가한다.
6. clock constraint를 지정한다.
7. synthesis를 실행한다.
8. timing, area, power report를 확인한다.
9. synthesized Verilog netlist를 export한다.
...
`step1_gemm_accelerator_top_mac*.v`는 `rtl_v2` dual-port GEMM accelerator 전용이다.
Oasys에서는 `step1.f`의 파일들을 추가한 뒤 다음 top module 중 하나를 선택한다.

```text
step1_gemm_accelerator_top_mac0  -> MAC_MODE=0
step1_gemm_accelerator_top_mac1  -> MAC_MODE=1
step1_gemm_accelerator_top_mac4  -> MAC_MODE=4
```

### 3.2 2단계 wrapper

`step2_gemm_accelerator_top_mac*.v`는 `rtl` single-port GEMM accelerator 전용이다.
`rtl` 계열에는 AT datapath가 없으므로 1-MAC과 4-MAC만 선택한다.

```text
step2_gemm_accelerator_top_mac1  -> MAC_MODE=1
step2_gemm_accelerator_top_mac4  -> MAC_MODE=4
```

### 3.3 3단계 wrapper

`step3_system_top_mac*.v`는 `rtl_v2` full-system top 전용이다.
Full-system 결과는 behavioral BRAM 영향을 받으므로 주 비교값이 아니라 참고값으로 둔다.

```text
step3_system_top_mac0  -> MAC_MODE=0
step3_system_top_mac1  -> MAC_MODE=1
step3_system_top_mac4  -> MAC_MODE=4
```
...
├── README.md
├── clk.sdc
├── step1_gemm_accelerator_top_mac0.v
├── step1_gemm_accelerator_top_mac1.v
├── step1_gemm_accelerator_top_mac4.v
├── step2_gemm_accelerator_top_mac1.v
├── step2_gemm_accelerator_top_mac4.v
├── step3_system_top_mac0.v
├── step3_system_top_mac1.v
├── step3_system_top_mac4.v
├── step1.f
├── step2.f
├── step3.f
```
...
| `step1_gemm_accelerator_top_mac*.v` | 1단계 `rtl_v2` accelerator mode top 제공 |
| `step2_gemm_accelerator_top_mac*.v` | 2단계 `rtl` accelerator mode top 제공 |
| `step3_system_top_mac*.v` | 3단계 `rtl_v2` system mode top 제공 |
