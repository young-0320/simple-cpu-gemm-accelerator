...
4개 명령 실행 후 생성된 VCD 경로가 Oasys config.tcl의 `vcd_file`과 일치하는지 확인한다. step2는 `asic/oasys/step2_mode{1,4}_config.tcl` 2개, step3는 같은 VCD(`step3_mode{1,4}_directed4x4x4/tb_gemm_system_v2.vcd`)를 원본 `step3_mode{1,4}_config.tcl`과 256-word 데모 `step3_mode{1,4}_demo_config.tcl` 양쪽에서 재사용하므로 4개, 총 **6개 config**에 반영한다.
