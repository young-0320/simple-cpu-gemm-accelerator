...
module gemm_system_top #(
    ...
    //   Depth reduced 4096 -> 256 for P&R demo: a full 4Kx32 memory maps to
    //   ~131072 flip-flops when synthesized as registers, which blows up the
    //   gate count and routing congestion. 256 words is plenty for the 4x4
    //   GEMM workload (A/B/C bases all fit under 0x100) and keeps the full
    //   CPU+GEMM+memory system small enough to place & route.
    // =======================================================
    reg [31:0] mem [0:255];
    reg [31:0] bram_rdata_a_r, bram_rdata_b_r;
    always @(posedge clk) begin
        if (bram_we_a) mem[bram_addr_a[7:0]] <= bram_wdata_a;
        bram_rdata_a_r <= mem[bram_addr_a[7:0]];
        bram_rdata_b_r <= mem[bram_addr_b[7:0]];
    end
