...
module gemm_mmio_reg (
    ...
    output wire        dim_oor,    // any of M/N/K written out of [1,4]
    ...
    reg [2:0]  r_m, r_n, r_k;
    // out-of-range flags: set when the *full 32-bit* written value is
    // outside [1,4]. Storing dims in 3 bits would alias large values
    // (e.g. 9 -> 1), so we judge the untruncated wdata at write time.
    reg        r_m_oor, r_n_oor, r_k_oor;
    ...
    assign n_dim  = r_n;
    assign k_dim  = r_k;
    assign dim_oor = r_m_oor | r_n_oor | r_k_oor;
