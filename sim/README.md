...
`sim/tb/tb_gemm_vectors_structured.sv`는 vector set을 transaction 단위로 replay하는 SystemVerilog transaction testbench이다. Generator, mailbox, driver, monitor, scoreboard 흐름으로 분리되어 있으며, DUT는 MMIO register와 external memory interface만 통해 검증한다.
...
| --- | --- | --- | --- |
| `structured` | `tb_gemm_vectors_structured.sv` | transaction-level 검증 기본 모드 | Yes |
| `legacy` | `tb_gemm_vectors.sv` | 기존 TB build/run smoke 확인 | No |

기본 모드는 `structured`이다. 보고서 산출물은 `structured` 모드에서만 정식으로 생성한다.

## 표준 실행 예시

아래 명령은 WSL Ubuntu shell에서 repository root로 이동한 뒤 실행한다.

```bash
cd /mnt/d/dev/rtl-dv-synthesis-lab/projects/simple-cpu-gemm-accelerator
```
...
python3 sim/scripts/run_gemm_verification.py \
  --vector-dir sim/vectors/directed_case \
  --run-id demo_directed_structured
```
...
| `--tb` | `structured` | `structured` 또는 `legacy` |
...
`structured` 모드의 산출물은 아래 경로에 생성된다.
...
```text
sim/results/directed_case/20260605_153000_structured/
```
...
run.log
report.md
tb_gemm_vectors_structured.fst
```

`metadata.json`은 run id, vector set, Verilator version, git commit, build/simulation command 같은 실행 metadata를 기록한다.

`summary.json`은 전체 transaction 수, pass/fail 수, timeout 수, cycle 요약, memory write count, C mismatch count를 기록한다.

`case_results.tsv`는 transaction당 한 줄의 검증 결과이다. 주요 컬럼은 `txn_name`, `m/n/k`, `expected_status`, `actual_status`, `cycles`, `mem_write_count`, `c_compare_count`, `c_mismatch_count`, `timeout`, `result`, `fail_reason`이다.
...
`run.log`는 simulation runtime 요약 로그이다. 각 transaction의 PASS/FAIL, cycle, memory write count, C mismatch count가 기록된다.

`report.md`는 위 산출물을 사람이 읽기 쉬운 보고서 형태로 묶은 최종 검증 보고서이다.

`tb_gemm_vectors_structured.fst`는 `--trace-fst`를 사용했을 때 유효한 waveform 파일이다. Trace를 켜지 않은 실행에서는 Verilator가 `$dumpvars`를 무시하므로 waveform 내용이 생성되지 않을 수 있다.
...
```bash
gtkwave sim/results/directed_case/<run_id>/tb_gemm_vectors_structured.fst
```

주로 볼 신호는 다음과 같다.

```text
tb_gemm_vectors_structured.mmio_*
tb_gemm_vectors_structured.mem_*
tb_gemm_vectors_structured.busy
tb_gemm_vectors_structured.state_debug
tb_gemm_vectors_structured.dut.u_fsm.*
tb_gemm_vectors_structured.dut.u_lsu.*
tb_gemm_vectors_structured.dut.g_mac4.u_mac.*
```
...
Structured verification runner smoke:

```bash
python3 sim/scripts/run_gemm_verification.py \
  --vector-dir sim/vectors/directed_case \
  --run-id smoke_structured
```
