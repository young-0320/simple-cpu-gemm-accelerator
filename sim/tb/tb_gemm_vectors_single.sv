...
// =======================================================
// tb_gemm_vectors_single
//   Single-port transaction replay testbench. The flow is split into
...
module tb_gemm_vectors_single #(
    ...
    initial begin
        ...
        if (!$value$plusargs("DUMPFILE=%s", dumpfile_path)) begin
            if (result_files_enabled) begin
                dumpfile_path = {result_dir, "/tb_gemm_vectors_single.fst"};
            end
            else begin
                dumpfile_path = "tb_gemm_vectors_single.fst";
            end
        end
        $dumpfile(dumpfile_path);
        $dumpvars(0, tb_gemm_vectors_single);

        log_line("== GEMM single-port transaction replay ==");
        ...
        if (total_errors != 0) begin
            $fatal(1, "GEMM single-port vector replay failed");
        end
        $finish;
    end
